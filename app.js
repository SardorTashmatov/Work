async function getDashboardData(url = '/data.json') {
    ...
    return data;
    // функция которая связывается с json
}

class DashboardItem {
    // тут говорится если при клике на daily то будет появятся daily
    static PERIODS = { 
        daily: 'day',
        weekly: 'week',
        monthly: 'month',
...
// создает разметку
    _createMarkup() {
        ...
        `);
// наверху он говорит где Last при клике на какуюто кнопку будет появлятся значние кнопки
...
// тут просто при клике на другую кнопку значение будет менятся
    changeView(view) {
